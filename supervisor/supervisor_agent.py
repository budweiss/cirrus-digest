...
heartbeat finds a new or retryable incident, escalates to a claude-agent-sdk reasoning pass
...
$150/month cap (CUMULUS.md sec 3) — when the cap is hit, no LLM call is made and
...
async def run_reasoning_pass(reason: str) -> float:
    ...
    if failed or cost is None:
        error = RuntimeError('supervisor reasoning pass did not complete successfully')
        error.cost = cost
        raise error
    return cost
...
def _handle_trigger(reason: str, is_daily: bool):
    ...
    try:
        ...
        if getattr(exc, 'cost', None) is not None:
            budget.record(exc.cost, detail=reason + ' (failed SDK pass)')
        ledger_append({"event": "reasoning-pass-failed", "tool": "supervisor_agent",
                       "detail": reason, "result": type(exc).__name__})
    if is_daily:
        _mark_daily_done()
    return success
...
def selftest() -> bool:
    import unittest
    import test_alert_policy
    result = unittest.TextTestRunner(verbosity=2).run(
        unittest.defaultTestLoader.loadTestsFromModule(test_alert_policy))

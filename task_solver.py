...
def solve_and_answer(rec: dict, creds: dict, to_addr: str, orig_subject: str) -> dict:
    ...
    # AFTER the send, deliberately: bookkeeping must never delay or block a
    # client's reply.
    _record_promise(rec, text, creds, orig_subject)
...
def selftest() -> int:
    ...
    try:
        ...
        # S78 — promise detection. The PREFILTER is the part worth testing
        # offline: it decides whether any model gets called at all, so a
        # regression there silently stops every promise being recorded.
        ...
        check("model JSON is parsed",
              _parse_promise_json('{"promise": true, "what": "the 224 workbook"}')
              == (True, "the 224 workbook"))
        check("a model wrapping JSON in prose still parses",
              _parse_promise_json('Sure. {"promise": false, "what": ""} done')
              == (False, ""))
        check("unusable model output returns None, not a false negative",
              _parse_promise_json("I think maybe yes") is None
              and _parse_promise_json("") is None)

...
class AdmissionTests(unittest.TestCase):
    def setUp(self):
        self.e=[dict(id='ollama',model='m',approved=True,task='t',capability='extract',
                     prompt_sha256=admission.prompt_digest('system'),evidence_id='review-1',
    ...
    def call(self):
        return admission.candidates(self.e,self.h,task='t',capability='extract',system='system',now=1000)
    ...
    def test_evaluation_scope_and_approval_required(self):
        for key,value in [('approved',False),('task','other'),('capability','other'),('prompt_sha256','changed'),('evidence_id','')]:
    ...
    def test_wrapper_passes_only_admitted_records(self):
        with patch('capability_dispatch.dispatch',return_value=('ollama','ok')) as dispatch:
            self.assertEqual(admission.dispatch_reviewed('system','user',{},evaluations=self.e,health=self.h,task='t',capability='extract',now=1000,max_cost_usd=0,privacy='LOCAL_ONLY'),('ollama','ok'))
